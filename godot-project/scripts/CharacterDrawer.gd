...
# --- 横向き 描画 ---
func _draw_side(m, p, d, skin_color, base_shirt_color, pants_color, skin_dark, _shirt_dark, pants_dark, shoulder_w, hip_w, thigh_w, shin_w, arm_w, neck_w):
    var cx = d["cx"]
    var cy = d["cy"]
    var wx = d["wx"]
    var wy = d["wy"]
