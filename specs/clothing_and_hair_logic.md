...
| ビュー | へそ | 腰(骨盤) | 肩 | 頭中心 |
|---|---|---|---|---|
| 側面 | `navel_x, navel_y` | `hip_x, hip_y` | `sx, sy` | `hx, hy` |
| 正面/背面 | `front_navel_x, front_navel_y` | `front_hip_x, front_hip_y` | `front_sx, front_sy` | `front_hx, front_hy` |

共通キー（全ビュー共通）:
- `cx, cy` — 股の座標
- `thigh_l`, `shin_l` — 太もも・すねの**長さ**（座標ではない）
- `navel_l`, `chest_l`, `hip_l` — 胴体各パーツの**長さ**（座標ではない）
- `waist_angle` — 腰の曲がり角度
...
### 4.2 下半身服（Bottoms）
腰（骨盤）（側面: `hip_x, hip_y` / 正面: `front_hip_x, front_hip_y`）から股（`cx, cy`）、そして足へと繋がる処理。
- **スカート（Skirt）**
  - **アンカー**: トップは腰(骨盤)座標を基準とする。ここに「ベルト」の描画を配置できる。
  - **形状**: 腰(骨盤)座標から、下方向に向かって末広がりになるポリゴン（台形や扇形）を動的に計算する。
...
### 4.3 上半身服（Tops）
肩（側面: `sx, sy` / 正面: `front_sx, front_sy`）からへそ（側面: `navel_x, navel_y` / 正面: `front_navel_x, front_navel_y`）までをカバーする処理。
...
- **袖部分**: `arm_l_angle` などの角度に沿って描画。
  ...
  - 「長袖」なら手首まで服の太さで描画する。
  - **輪郭線**: 背景や同系色の服と同化するのを防ぐため、袖にあたる部分のみ薄いグレー（半透明）のアウトラインを付ける。
  - **側面ビューの調整（すぼみ）**: 横向きの際、袖口が全体的に太すぎないようにするため、背中側の描画位置を固定したまま胸側の太さを削る（テーパードのような）調整を行う。
...
3. **スカートの試験実装**
   一番特徴的となる「腰（骨盤）基準で広がるスカート形状」を描画する関数をテスト的に追加。
4. **袖（半袖／長袖）の試験実装**
